...
const Profile = () => {
  ...
  // Simulate login (in a real scenario, this would come from authentication)
  const handleLogin = () => {
    setIsLoggedIn(true); // Set user as logged in
    setIsSlidingOpen(false); // Close the sliding section after login
  };
    ...
    <div className='flex flex-col pt-24'>
      <div className='p-4 border-b'>
        ...
        {isLoggedIn ? (
          ...
          <p>Sign in to start your healthcare journey</p>
        )}
        {!isLoggedIn && (
          <button onClick={toggleSlide} className='font-bold w-full px-auto mt-2 py-2 text-white bg-red-500 rounded-lg'>
            Sign in
          </button>
        )}
      </div>
      ...
      <div className='flex flex-col justify-center border-b-8 border-gray-100 py-3'>
        ...
        <LoginSlidingSection isOpen={isSlidingOpen} toggleSlide={toggleSlide} onLogin={handleLogin} />
      </div>
    </div>
  );
};
